...
# https://web.archive.org/web/20220521003827/https://www.covid19.admin.ch/api/data/documentation/
# extended hosp & death data by vaccination status with records for the newly added vaccination_
# status of fully_vaccinated_no_booster and fully_vaccinated_first_booster which are a more
# detailed subdivision of the records with vaccination_status fully_vaccinated. the records with
# vaccination_status fully_vaccinated will continue to be published as they have been until now.
# this affects all *_vaccpersons.(json/csv)files but will only be added for records with vaccine all.
...
df <- read.csv("./data_static/COVID19Death_vaccpersons_AKL10_w.csv") |>
  ...
  as_tibble() |>
  # filter(vaccination_status %in% c(
