...
make_chart <- function(data, title) {
  ggplot(
    data,
    aes(x = date, y = asmr_py, group = vaccination_status, color = vaccination_status)
  ) +
    labs(
      title = title,
...
acm <- df |>
  ...
  select(-type)
make_chart(acm, "Monthly All-Cause ASMR-PY by Vaccination [UK]")
...
acm_non_covid <- df |>
  ...
  select(-type)
make_chart(acm_non_covid, "Monthly Non-COVID-19 ASMR-PY by Vaccination [UK]")
...
covid <- df |>
  ...
  select(-type)
make_chart(covid, "Monthly COVID-19 ASMR-PY by Vaccination [UK]")


# Adjust rates for bias, via weekly ratio
ratio <- acm_non_covid |>
  select(1, 2, 3) |>
  pivot_wider(names_from = vaccination_status, values_from = asmr_py) |>
...
acm_adjusted <- rbind(
  acm |> filter(vaccination_status == "Ever vaccinated"),
  unvaxx_adjusted
) |> arrange(date)
make_chart(acm_adjusted, "Monthly Adjusted All-Cause ASMR-PY by Vaccination [UK]")
